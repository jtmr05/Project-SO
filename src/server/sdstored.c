...
void handle_sigalrm(){
    ...
    while((pid_filho = waitpid(-1, &status, WNOHANG)) > 0){
        ...
        if(WEXITSTATUS(status) == 0){
            ...
            int bytes_fd = open("./tmp/bytes_fifo",O_WRONLY);
            struct stat stats_in;
            struct stat stats_out;


            if (stat(process->input_file, &stats_in) == 0 && stat(process->output_file, &stats_out) == 0)
            {   char*buffer = malloc(1024);
                sprintf(buffer,"%ld;%ld",stats_in.st_size,stats_out.st_size);
                write(bytes_fd,buffer,strlen(buffer));
                free(buffer);
...
int main(int argc, char *argv[]){
    ...
    if(mkfifo("./tmp/bytes_fifo", 0644) == -1) {perror("Fifo"); return -1;}
    //Sinais
